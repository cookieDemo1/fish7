	...
	<page-container>
		<div class="page-inner">
			<card-container class="card-container" :show-header="false">
				<div class="task">
					<div class="title">
						<svg-icon class="icon" name="zrenwu_icon"></svg-icon>子任务-{{ type === 'add' ? '新增' : '修改' }}
					</div>
					<div class="container">
						<div class="content">
							<m-form ref="formRef" class="form" :model="form" :rules="rules">
								<m-form-item label="" name="name">
									<m-input v-model="form.name" placeholder="请填写子任务名称" :maxlength="20"></m-input>
								</m-form-item>
								<m-form-item class="m-form-item-extra" label="执行操作" name="ope">
									...
									<template v-else>
										...
										<add-actions type="addDevice" @callback="item => form.ope.push(item)"></add-actions>
									</template>
								</m-form-item>
							</m-form>
						</div>
