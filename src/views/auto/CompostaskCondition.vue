	...
	<page-container>
		<div class="page-inner">
			<card-container class="card-container" :show-header="false">
				<div class="task">
					<div class="title">
						<svg-icon class="icon" name="tiaojrw_icon"></svg-icon>条件任务-{{ type === 'add' ? '新增' : '修改' }}
					</div>
					<div class="container">
						<div class="content">
								...
								<m-form-item label="执行条件" name="sensor">
									<div class="condition-card">
										...
										<div class="condition-item">
											...
											<a-row :gutter="[22, 22]">
												<a-col :span="8">
													<m-select v-model="form.arg" class="m-select-compact" placeholder="请选择" :options="level_2">
													</m-select>
													<div class="line"></div>
...
<script setup lang="ts">
	...
	const { autoTaskConditionOption, getAutoTaskConditionOption } = use.useMainStateAction('autoTaskConditionOption');
	...
	const level_2 = computed(() => {
		if (level_1.value.length && form.sensor_id) {
			...
			return (level_2_obj.value[type] || []).map(item => ({ ...item, label: item.v, value: item.k }));
		}
		return [];
	});
