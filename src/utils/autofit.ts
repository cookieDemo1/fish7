...
const autofit: Autofit = {
  ...
  off(el = 'body') {
    try {
      ...
      const temp = document.querySelector<HTMLDivElement>(
        currRenderDom ? (currRenderDom as string) : el
      )
      if (temp) {
        // @ts-ignore
        temp.style = ''
...
function elRectification(
  el: string,
  isKeepRatio: string | boolean = true,
  ...
  const currEl = Array.from(
    document.querySelectorAll<HTMLElement & { originalWidth: number; originalHeight: number }>(el)
  )
  if (currEl.length == 0) {
...
function keepFit(
  ...
  const normalRatio = dw / dh
  if (ratio >= normalRatio - 0.02 && ratio <= normalRatio + 0.02) {
    currScale = clientWidth / clientHeight < dw / dh ? clientWidth / dw : clientHeight / dh
    dom.style.overflow = 'hidden'
  } else if (ratio < normalRatio - 0.02) {
    currScale = clientHeight / dh
    dom.style.overflowX = 'auto'
    dom.style.overflowY = 'hidden'
  } else {
    currScale = clientWidth / dw
    dom.style.overflowX = 'hidden'
    dom.style.overflowY = 'auto'
  }

  const height = clientHeight / currScale
  const width = clientWidth / currScale
