			...
			<div class="card-body">
				<div class="info">
					<div class="left">
						...
						<div class="bottom" :class="{ yellow: item.color == 2, red: item.color == 3, blue: item.color == 4 }">
							<div v-if="item.status != '2'" class="state">{{ item.level || '--' }}</div>
							<div class="name">{{ item.name }}</div>
						</div>
					</div>
					<div class="right">
						<SvgIcon class="icon" :name="item.status == '2' ? item.icons[0] : item.icons[item.color]"></SvgIcon>
					</div>
				</div>
